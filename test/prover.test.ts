...
import { SnarkProof, REGISTRY_TREE_HEIGHT, HydraS1Prover, buildPoseidon, Poseidon, ACCOUNTS_TREE_HEIGHT, KVMerkleTree, MerkleTreeData, EddsaPublicKey, HydraS1Account } from "../package/src";
...
describe("Hydra S1 Prover", () => {
  ...
  before(async () => {
    ...
    isStrict = registryTree.getValue(accountsTree1.getRoot().toHexString()).toNumber() as 0 | 1;
  })
    ...
    it("Should generate a snark proof with correct inputs", async () => {
      ...
      expect(snarkProof.input).to.deep.equal([
          ...
          '16663346758351127135188808412939519675738450963117260004536366823769793157136',
          '123',
    ...
    it("Should export the proof in Bytes", async () => {
      expect(snarkProof.toBytes().substring(514)).to.equal("")
    });
    ...
    it("Should throw when the external nullifier overflow the snark field", async () => {
      const ticketIdentifierOverflow = "0x48c8947f69c054a5caa934674ce8881d02bb18fb59d5a63eeaddff735b0e9801e87294783281ae49fc8287a0fd86779b27d7972d3e84f0fa0d826d7cb67dfefc";
      try{
        ...
        expect(e.message).to.equal("External nullifier overflow the snark field, please use external nullifier inside the snark field")
      }
    });
    ...
    it("Should throw when sending claimedValue > sourceValue", async () => {
      try{
        ...
        expect(e.message).to.equal(`Claimed value ${BigNumber.from(10).toHexString()} can't be superior to Source value`);
      }
    });
    ...
    it("Should throw when sending claimedValue is not equal to sourceValue and isStrict == 1", async () => {
      try{
        ...
        expect(e.message).to.equal(`Claimed value ${BigNumber.from(3).toHexString()} must be equal with Source value when isStrict == 1`);
      }
    });
    ...
    it("Should throw when sending claimedValue negative", async () => {
      try{
        ...
        expect(e.message).to.equal(`Claimed value ${BigNumber.from(-3).toHexString()} can't be negative`);
      }
    });
    ...
    it("Should throw when sending a source which is not in the accountsTree", async () => {
      ...
      try{
        ...
        expect(e.message).to.equal(`Could not find the source ${BigNumber.from(newSource.identifier).toHexString()} in the Accounts tree`);
      }
    });
})