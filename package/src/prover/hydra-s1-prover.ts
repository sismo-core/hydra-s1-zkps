...
import { zkeyPath, wasmPath } from "./files";
...
export class HydraS1Prover {
  ...
  public async generateInputs({
    ...
    const accountMerklePath = accountsTree.getMerklePathFromKey(source.identifier.toHexString());
    const sourceValue = accountsTree.getValue(source.identifier.toHexString());

    const registryMerklePath = this.registryTree.getMerklePathFromKey(
      accountsTree.getRoot().toHexString()
    );
    const accountsTreeValue = this.registryTree.getValue(accountsTree.getRoot().toHexString());
    ...
    const privateInputs: PrivateInputs = {
      ...
      sourceCommitmentReceipt: source.commitmentReceipt.map(el => BigNumber.from(el).toBigInt()),
      destinationSecret: destination.secret.toBigInt(),
      destinationCommitmentReceipt: destination.commitmentReceipt.map(el => BigNumber.from(el).toBigInt()),
      accountsTreeRoot: accountsTree.getRoot().toBigInt(),
      accountMerklePathElements: accountMerklePath.elements.map(el => el.toBigInt()),
      accountMerklePathIndices: accountMerklePath.indices,
      registryMerklePathElements: registryMerklePath.elements.map(el => el.toBigInt()),
    ...
    const publicInputs: PublicInputs = {
      ...
      commitmentMapperPubKey: this.commitmentMapperPubKey.map(el => el.toBigInt()),
  ...
  public async userParamsValidation({
    ...
    const isSourceCommitmentValid = await verifyCommitment(source.identifier, source.secret, source.commitmentReceipt, this.commitmentMapperPubKey);
    if (!isSourceCommitmentValid) throw new Error("Invalid source commitment receipt");

    const isDestinationCommitmentValid = await verifyCommitment(destination.identifier, destination.secret, destination.commitmentReceipt, this.commitmentMapperPubKey);
    if (!isDestinationCommitmentValid) throw new Error("Invalid destination commitment receipt");

    let sourceValue;
    try {
      sourceValue = accountsTree.getValue(source.identifier.toHexString()).toNumber();
    } catch (e) {
      throw new Error(`Could not find the source ${source.identifier.toHexString()} in the Accounts tree`);
    }

    if (claimedValue.gt(sourceValue)) {
      throw new Error(`Claimed value ${claimedValue.toHexString()} can't be superior to Source value`);
    }

    if (isStrict && !claimedValue.eq(sourceValue)) {
      throw new Error(`Claimed value ${claimedValue.toHexString()} must be equal with Source value when isStrict == 1`);
    } 

    if (claimedValue.lt(0)) {
      throw new Error(`Claimed value ${claimedValue.toHexString()} can't be negative`);
    }

    const SnarkField = BigNumber.from(SNARK_FIELD);
    if (ticketIdentifier.gt(SnarkField)) {
      throw new Error("External nullifier overflow the snark field, please use external nullifier inside the snark field");
    }
    if (source.identifier.gt(SnarkField)) {
      throw new Error("Source identifier overflow the snark field, please use source identifier inside the snark field");
    }
    if (source.secret.gt(SnarkField)) {
      throw new Error("Source secret overflow the snark field, please use source secret inside the snark field");
    }
    if (destination.identifier.gt(SnarkField)) {
      throw new Error("Destination overflow the snark field, please use destination inside the snark field");
    }
    if (destination.secret.gt(SnarkField)) {
      throw new Error("Destination secret overflow the snark field, please use destination secret inside the snark field");
    }
    if (claimedValue.gt(SnarkField)) {
      throw new Error("Claimed value overflow the snark field, please use claimed value inside the snark field");
    }
  }
